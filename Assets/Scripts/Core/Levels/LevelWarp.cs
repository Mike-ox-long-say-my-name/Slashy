...
namespace Core.Levels
{
    ...
    public class LevelWarp : MonoBehaviour
    {
        ...
        {
            var movement = playerInfo.Player.PlayerMovement;
            var player = movement.BaseMovement.Transform;

            while (true)
            {
                var targetDirection = (target - player.position);
                targetDirection.y = 0;
                targetDirection.Normalize();
                movement.Move(targetDirection);
                var playerNewPosition = player.position;
        ...
        private static void OnSceneLoaded(Scene scene, LoadSceneMode mode)
        {
            ...
            movement.BaseMovement.SetPosition(_transferData.StartPosition);
